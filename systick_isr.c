...
volatile uint32_t ticks = 0;	/* Variable to store millisecond ticks */
...
void SysTick_Handler(void){		// SysTick interrupt service routine
	if(TimingDelay != 0x00)			// prevent it from becoming negative
		TimingDelay--;
...
void Delay(uint32_t nTime){
	...
	TimingDelay = nTime;			// TimingDelay declared volatile
